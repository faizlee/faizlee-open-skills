...
$ccSessionName = if ($CcSessionName) { $CcSessionName } elseif ($detectedCcSessionId) { $detectedCcSessionId } elseif ($env:WT_SESSION) { "WT:$($env:WT_SESSION)" } else { "$env:USERNAME@$env:COMPUTERNAME" }
...
- 如果 ccSessionId 为空，面板无法恢复对应 Claude Code 会话；面板创建/重绑 pair 时会优先生成明确的 Claude Code session id。
- 不使用 --last。
- 不使用 subagent。
- 不启动 codex-with-cc。
"@
